...
def VaR_MonteCarlo(w:pd.Series, rend:pd.DataFrame, alpha:float = 0.95, n_sim:int = 10000, seed:int = 42) -> tuple[float,float]:
    ...
    mu = rend.mean()
    Sigma = rend.cov()
    # Generator con seed esplicito: garantisce che la simulazione sia riproducibile
    # (la stessa esecuzione produce gli stessi VaR/ES, non lo stato globale di numpy)
    rng = np.random.default_rng(seed)
    rend_sim = rng.multivariate_normal(mean=mu, cov=Sigma, size=n_sim)
    rend_ptf = rend_sim@w
